...
4. 如果需要机器专用运行时配置，请把改动写进 `config.yaml`，然后按需修改 `template/general.yaml` 或 `template/zju.yaml`。服务使用 `DEFAULT_TEMPLATE` 指定的模板，默认值为 `general`。`HOST` 和 `PORT` 也来自 `config.yaml`。详细模板项可参考 [配置](../configuration/overview)
5. 运行程序。
